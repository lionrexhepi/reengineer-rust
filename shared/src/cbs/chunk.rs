...
use crate::{ dimension::chunk::{ SubChunk, Chunk }, util::WriteExt };
...
impl Packetable for Chunk {
    fn write_to_buffer<T: Write + Unpin + Send>(
        ...
        let available_subchunks = 0u16; //16 bit flags; the nth bit is a bool whether or not that subchunk is included or not

        let mut subchunks: Vec<(&u8, &SubChunk)> = self.non_air_sub_chunks.iter().collect();
    ...
    fn read_from_buf(reader: &mut super::PacketBuf) -> anyhow::Result<Self> where Self: Sized {
        ...
        for i in 0..15 {
            if available_subchunks & (1 << i) != 0 { //Check each bit for being true
